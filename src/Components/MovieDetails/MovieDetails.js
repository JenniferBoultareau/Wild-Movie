...
class MovieDetails extends Component {
  ...
  componentDidMount() {
    if(localStorage.getItem('posters')) {
      this.setState({posters: JSON.parse(localStorage.getItem('posters'))})
    } else {
    localStorage.setItem('posters', JSON.stringify(this.imgMovie));
    this.setState({posters:(this.imgMovie)})
    }
  }
